...
def r_msd_single(SIMS,LEG):
    ...
    for count in range(len(SIMS)):
        ...
        [fig,ax]=a.plot_fit([fig,ax],mx,my,mxn,myn2,lab,'fit',1)
        r.curdir=r.curdir=r.chandra+"msd_04_Dec"
        r.mkdir(r.curdir)
        r.curdir=r.curdir+"/AM3"
        r.mkdir(r.curdir)
        [fig,ax]=a.titler([fig,ax],"Simname:"+sim+"Exponent"+str(po))
        a.axis_modify([fig,ax],r.curdir,r'$log_{10}(t)$',r'$log_{10} (MSD)$','msdr',1)
r_msd_single(['B121'],[r'$N=12 \, D=1.3 \mu m^2/s$'])
